...
// ── sec.waf.apply (PRD §20) ───────────────────────────────────────────────────

const wafConfPath = "/etc/nginx/jawaker/waf.conf"
const wafConfBak = "/etc/nginx/jawaker/waf.conf.bak"

// SecWAFApply generates an nginx WAF snippet and reloads nginx.
// It writes a backup before overwriting, and restores the backup if nginx
// refuses the new configuration.
func (e *Executors) SecWAFApply(ctx context.Context, in nodewire.SecWAFApplyInput) (nodewire.SecWAFApplyResult, error) {
	if !supportedOS() {
		return nodewire.SecWAFApplyResult{Applied: true, RulesCount: len(in.Rules)}, nil
	}

	nginx := resolveTool(nginxBinaryCandidates)
	if nginx == "" {
		return nodewire.SecWAFApplyResult{}, notAvailable("nginx is not installed on this host")
	}

	snippet := buildWAFSnippet(in.Rules)

	// Backup existing file (best-effort).
	if _, err := os.Stat(wafConfPath); err == nil {
		_ = os.Rename(wafConfPath, wafConfBak)
	}

	if err := os.MkdirAll("/etc/nginx/jawaker", 0o750); err != nil {
		return nodewire.SecWAFApplyResult{}, fmt.Errorf("sec.waf.apply: mkdir: %w", err)
	}
	if err := os.WriteFile(wafConfPath, []byte(snippet), 0o640); err != nil {
		return nodewire.SecWAFApplyResult{}, fmt.Errorf("sec.waf.apply: write: %w", err)
	}

	// Test nginx config.
	e.spawns.Add(1)
	testRes, testErr := e.cmdRunner(ctx, CommandSpec{
		Path: nginx,
		Args: []string{"-t"},
	})
	if testErr != nil || strings.Contains(testRes.Stderr, "[error]") {
		// Restore backup on failure.
		_ = os.Rename(wafConfBak, wafConfPath)
		return nodewire.SecWAFApplyResult{}, fmt.Errorf("sec.waf.apply: nginx -t failed: %s", strings.TrimSpace(testRes.Stderr))
	}
	...
	return nodewire.SecWAFApplyResult{
		Applied:    true,
		RulesCount: len(in.Rules),
		Message:    "WAF rules applied and nginx reloaded",
	}, nil
}

// buildWAFSnippet generates an nginx include snippet from rule list.
// Only IP/CIDR deny rules are handled; other kinds are skipped with a comment.
func buildWAFSnippet(rules []nodewire.WAFRule) string {
	var sb strings.Builder
	sb.WriteString("# Generated by JAWAKER sec.waf.apply — do not edit manually\n")
	for _, r := range rules {
		action := r.Action
		if action == "" {
			action = "deny"
		...
		case "ip", "cidr", "":
			if action == "deny" {
				sb.WriteString("deny " + r.Pattern + ";\n")
			} else if action == "allow" {
				sb.WriteString("allow " + r.Pattern + ";\n")
			}
		default:
			sb.WriteString("# unsupported rule kind " + r.Kind + " skipped\n")
		}
	}
	if len(rules) > 0 {
		sb.WriteString("deny all; # default drop not in allow list\n")
	}
	return sb.String()
}
