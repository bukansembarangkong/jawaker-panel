...
func TestMain(m *testing.M) {
	...
	dsn, err := dbtest.IsolatedDatabase(base, "jobs")
	if err != nil {
		fmt.Fprintf(os.Stderr, "jobs: isolated test database: %v\n", err)
		os.Exit(1)
	}
	if err := os.Setenv("JAWAKER_TEST_DATABASE_URL", dsn); err != nil {
		fmt.Fprintf(os.Stderr, "jobs: set test database: %v\n", err)
		os.Exit(1)
	}

	code := m.Run()
	_ = dbtest.DropDatabase(base, "jobs")
	os.Exit(code)
}
