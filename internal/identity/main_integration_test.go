...
//
// go test runs packages in parallel, and both this suite and the migrate
...
func TestMain(m *testing.M) {
	...
	dsn, err := dbtest.IsolatedDatabase(base, "identity")
	if err != nil {
		fmt.Fprintf(os.Stderr, "identity: isolated test database: %v\n", err)
		os.Exit(1)
	}
	if err := os.Setenv("JAWAKER_TEST_DATABASE_URL", dsn); err != nil {
		fmt.Fprintf(os.Stderr, "identity: set test database: %v\n", err)
		os.Exit(1)
	}
	...
	_ = dbtest.DropDatabase(base, "identity")
	os.Exit(code)
}
