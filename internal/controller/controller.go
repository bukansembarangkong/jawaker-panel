...
type Handler struct {
	...
	// Containers is the Phase 9 container platform inventory store.
	Containers *containers.Store
	// ContainerRoutesMounted reports whether /api/v1/projects/{project_id}/container-*
	// is registered.
	ContainerRoutesMounted bool
	// Network is the Phase 10 networking store (firewall rules, port forwards,
	// wireguard peers, apply log).
...
func Build(opts Options) (*Handler, error) {
	...
	default:
		...
		// Container platform (Phase 9): registries, compose stacks, containers,
		// volumes, and live log tails via nodewire OpContainerLogs.
		out.Containers = containers.NewStore(opts.DB, out.Secrets)
		containerHandlers, containerErr := NewContainerHandlers(ContainerHandlerOptions{
		...
		if containerErr != nil {
			return nil, fmt.Errorf("controller: container handlers: %w", containerErr)
		}
		containerRoutes := containerHandlers.Routes

		// Networking (Phase 10): firewall rules, port forwards, zones,
		// wireguard peers, port inventory, diagnostics, apply log.
		out.Network = network.NewStore(opts.DB)
		networkHandlers, networkErr := NewNetworkHandlers(NetworkHandlerOptions{
			Network:    out.Network,
			Pool:       opts.DB,
			Dispatcher: out.Dispatcher,
			Logger:     logger,
		...
		if networkErr != nil {
			return nil, fmt.Errorf("controller: network handlers: %w", networkErr)
		}
		...
		quotaHandlers, quotaErr := NewQuotaHandlers(QuotaHandlerOptions{
			Pool:   opts.DB,
			Logger: logger,
			Audit:  opts.DB,
			Now:    now,
		...
		if quotaErr != nil {
			return nil, fmt.Errorf("controller: quota handlers: %w", quotaErr)
		}
