...
function routeFromHash(): Route {
  ...
  if (hash.startsWith('#/security-center')) return 'security-center';
  if (hash.startsWith('#/security')) return 'security';
  if (hash.startsWith('#/servers')) return 'servers';
  if (hash.startsWith('#/sites')) return 'sites';
  if (hash.startsWith('#/apps')) return 'apps';
  if (hash.startsWith('#/databases')) return 'databases';
  if (hash.startsWith('#/backups')) return 'backups';
  if (hash.startsWith('#/observability')) return 'observability';
  if (hash.startsWith('#/dnstls')) return 'dnstls';
  if (hash.startsWith('#/containers')) return 'containers';
  ...
  if (hash.startsWith('#/workers')) return 'workers';
  if (hash.startsWith('#/files')) return 'files';
  if (hash.startsWith('#/settings')) return 'settings';
  if (hash.startsWith('#/dr-wizard')) return 'dr-wizard';
  if (hash.startsWith('#/automation')) return 'automation';
  if (hash.startsWith('#/audit-log')) return 'audit-log';
  return 'dashboard';
}
  ...
  const navSections: NavSection[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { route: 'security', label: 'My Security', icon: 'M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z' },
