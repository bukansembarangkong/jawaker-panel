...
-- Tables: hardening_checks, security_events, ban_entries, waf_rules, ssh_posture_log

-- Hardening check results per server
CREATE TABLE hardening_checks (
    id              TEXT        NOT NULL DEFAULT gen_random_id(),
    server_id       TEXT        NOT NULL REFERENCES servers(id) ON DELETE CASCADE,
    check_name      TEXT        NOT NULL,
    category        TEXT        NOT NULL DEFAULT 'os',      -- os | ssh | firewall | packages | services
    severity        TEXT        NOT NULL DEFAULT 'info',    -- critical | high | medium | low | info
    status          TEXT        NOT NULL DEFAULT 'pass',    -- pass | fail | warn | skip
    title           TEXT        NOT NULL,
...
CREATE TABLE security_events (
    id          TEXT        NOT NULL DEFAULT gen_random_id(),
    server_id   TEXT        NOT NULL REFERENCES servers(id) ON DELETE CASCADE,
    source      TEXT        NOT NULL DEFAULT 'system',   -- crowdsec | fail2ban | sshd | system
    kind        TEXT        NOT NULL DEFAULT 'auth_fail', -- auth_fail | ban | unban | intrusion | anomaly
...
CREATE TABLE ban_entries (
    ...
    server_id   TEXT        NOT NULL REFERENCES servers(id) ON DELETE CASCADE,
    ip          TEXT        NOT NULL,
    source      TEXT        NOT NULL DEFAULT 'manual',   -- crowdsec | fail2ban | manual
...
CREATE TABLE waf_rules (
    id          TEXT        NOT NULL DEFAULT gen_random_id(),
    server_id   TEXT        NOT NULL REFERENCES servers(id) ON DELETE CASCADE,
    kind        TEXT        NOT NULL DEFAULT 'rate_limit', -- rate_limit | block_ua | block_ip | custom
